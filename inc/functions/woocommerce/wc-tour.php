...
function tf_tours_booking_function()
{
    ...
    $tour_time =  !empty($_POST['check-in-time']) ? sanitize_text_field($_POST['check-in-time']) : '';
    // Tour extra
     ...
     * 
     */
    // If no people selected from the forntend
    if (!$total_people) {
        $response['errors'][] = __('You must select people!', 'tourfic');
    }
    if ($tour_type == 'fixed') {
...
/**
 * look up if booking has seasional price
 * @return bool
 */
function tf_find_availability($start)
{
}
